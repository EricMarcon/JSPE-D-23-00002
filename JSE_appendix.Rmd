...
# Theoretical example: non-homogeneous controls{#sec:thnonh}
...
The south-eastern group of points illustrates that the relative concentration of cases can be below 1 when only a few cases are present among many controls.
...
To test the intertype concentration between sane and infested maple trees, the intertype *M* statistic is computed.
...
Infested and sane maple trees are significantly concentrated.
The map shows that the local concentration of maples around infested trees is generally high (warm colors around black points). 

In contrast, decaying trees and maples rather, but non significantly, repulse each others: the intertype $M$ function between them is plotted below:
