...
points(CASES, pch = 20)
# Plot high quantiles in red. 
# Significant values are store in "High" and "Low" attributes of the image
points(
...
Each individual value of $M$ is compared to its distribution under the null hypothesis of random location of all other points, obtained by Monte-Carlo simulations.
A map is produced.

```{r}
# Calculate individual M with quantiles 
trees_2021 %>% 
  Mhat(
    r = c(0, Distance),
    ReferenceType = "Acer",
...
points(
  trees_2021[trees_2021$marks$PointType == "Acer"][attr(map_acer, "High")],
  pch = 20, col = "green"
)
# And decaying trees
points(
...
Considering 6-meter-radius neighborhoods, the map is difficult to read because it shows local, highly variable, information that hides the main tendencies. 
...
Low concentration areas, where $M<1$ are masked.
